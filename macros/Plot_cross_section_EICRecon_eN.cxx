...
void Plot_cross_section_EICRecon_eN(const int plot_RC_flag = 1, const int plot_ACC_flag = 1, TString inFileName = "Input")
{
  ...
  float sys_err_proton = 0.02; //2% for proton transport matrix
  
  float sys_err_smear = sys_err_proton;

  float sys_err_tot_corr = sqrt(sys_err_lumi*sys_err_lumi + sys_err_theory*sys_err_theory + sys_err_beam_eff*sys_err_beam_eff + sys_err_proton*sys_err_proton );
  float sys_err_tot_corr_D = sys_err_lumi;
  ...
  float alpha_n_min = 0.99;
  float alpha_n_max = 1.01;
  float Delta_apha_p = alpha_n_max - alpha_n_min;
  ...
  if(!inFile->IsOpen())
  {
    cout<<"File not open correctly. Check file name or path."<<endl;
    
    return;  
  }

  ofstream outputFile("./output/n_cross_section.txt");
  ofstream outputFile_D("./output/D_cross_section_eN.txt");
  ...
  //this is for clone of TH1F *h_sigma_n_red[nx_bins][nQ2_bins]; for new systematic errors
  //Needs to be TH1D for Clone
  const int n_variations = 10; //number of variations for sys. err.

  //TH1D *h_sigma_n_red_var[nx_bins][nQ2_bins][n_variations];

  //----------------------------

  TH2F *h_y_vs_x_vs_Q2_RC_cuts_pT2_cut = (TH2F*)inFile->Get("h_y_vs_x_vs_Q2_RC_cuts_pT2_cut");
  ...
  TGraphErrors *sigma_free_n_vs_x[nQ2_bins];
  TGraphErrors *sigma_free_n_vs_x_RC_corr[nQ2_bins];


  TGraphErrors *sigma_D_vs_x[nQ2_bins];
  TGraphErrors *sigma_D_vs_x_RC_corr[nQ2_bins];
  ...
  TCanvas *sigma_red_n_vs_x_one_can = new TCanvas("sigma_red_n_vs_x_one_can", "sigma_red_n_vs_x_one_can", 2400,2000);
  sigma_red_n_vs_x_one_can->Divide(2,2);

  TCanvas *sigma_red_n_vs_x_one_can_2 = new TCanvas("sigma_red_n_vs_x_one_can_2", "sigma_red_n_vs_x_one_can_2", 4800,1000);
  ...
  TCanvas *sigma_red_D_vs_x_one_can = new TCanvas("sigma_red_D_vs_x_one_can", "sigma_red_D_vs_x_one_can", 2400,2000);
  sigma_red_D_vs_x_one_can->Divide(2,2);

  TCanvas *sigma_red_D_vs_x_one_can_2 = new TCanvas("sigma_red_D_vs_x_one_can_2", "sigma_red_D_vs_x_one_can_2", 4800,1000);
  TPad *pad_0_D = new TPad("pad_0_D", "pad_0_D", 0.0, 0.0, 0.28, 1.0);
  pad_0_D->Draw();
  TPad *pad_1_D = new TPad("pad_1_D", "pad_1_D", 0.28, 0.0, 0.52, 1.0);
  pad_1_D->Draw();
  TPad *pad_2_D = new TPad("pad_2_D", "pad_2_D", 0.52, 0.0, 0.76, 1.0);
  pad_2_D->Draw();
  TPad *pad_3_D = new TPad("pad_3_D", "pad_3_D", 0.76, 0.0, 1.0, 1.0);
  pad_3_D->Draw();
  ...
  float statistics_scale_D = lumi_design_fb/lumi_fb; //design lumi is for eN+eP, we have just 1/2


  for(unsigned int Q2_bin = 0; Q2_bin < nQ2_bins; Q2_bin++)
  {
    outputFile<<Q2_bins[Q2_bin]<<" < Q2 < "<<Q2_bins[Q2_bin+1]<<" GeV^2"<<endl;
    outputFile<<"x  sigma_n,red  stat_err  tot_err  y"<<endl;
    ...
    sigma_free_n_vs_x[Q2_bin] = new TGraphErrors(nx_bins);
    sigma_free_n_vs_x[Q2_bin]->SetNameTitle(Form("g_sigma_n_red_vs_x_Q2bin_%i", Q2_bin));

    sigma_free_n_vs_x_RC_corr[Q2_bin] = new TGraphErrors(nx_bins);
    sigma_free_n_vs_x_RC_corr[Q2_bin]->SetNameTitle(Form("g_sigma_n_red_vs_x_Q2bin_%i_RC_corr", Q2_bin));

    //---

    sigma_D_vs_x[Q2_bin] = new TGraphErrors(nx_bins);
    sigma_D_vs_x[Q2_bin]->SetNameTitle(Form("g_sigma_D_vs_x_Q2bin_%i", Q2_bin));

    sigma_D_vs_x_RC_corr[Q2_bin] = new TGraphErrors(nx_bins);
    sigma_D_vs_x_RC_corr[Q2_bin]->SetNameTitle(Form("g_sigma_D_vs_x_Q2bin_%i_RC_corr", Q2_bin));
    ...
    TH1D* h_sigma_D_red_vs_x_RC_sys = (TH1D*)h_sigma_D_red_vs_x[Q2_bin]->Clone(Form("h_sigma_D_red_vs_x_RC_sys_%i", Q2_bin));
    ...
    for(unsigned int x_bin = 0; x_bin < nx_bins; x_bin++)
    {
      ...
      float y_mean = h_y_vs_x_vs_Q2_RC_cuts_pT2_cut->GetBinContent(x_bin+1,Q2_bin+1);
      ...
      h_sigma_n_red[x_bin][Q2_bin] = (TH1F*)inFile->Get(Form("h_sigma_n_red_xbin_%i_Q2bin_%i", x_bin, Q2_bin));
      h_sigma_n_red_RC[x_bin][Q2_bin] = (TH1F*)inFile->Get(Form("h_sigma_n_red_RC_xbin_%i_Q2bin_%i", x_bin, Q2_bin));
      h_sigma_n_red_MC_ACC[x_bin][Q2_bin] = (TH1F*)inFile->Get(Form("h_sigma_n_red_MC_ACC_xbin_%i_Q2bin_%i", x_bin, Q2_bin));
      ...
      outputFile_D<<x_bin_center<<" "<<sigma_D_red_MC<<" "<<sigma_D_red_MC_err<<" "<<sigma_D_red_MC*sys_err_tot_corr_D<<endl;

      //-----------------

      if( h_sigma_n_red[x_bin][Q2_bin]->Integral() < 1 ) continue; //skip bins with too low statistics

      //----------------

      sigma_D_vs_x[Q2_bin]->SetPoint(x_bin, x_bin_center, sigma_D_red_MC);
      sigma_D_vs_x[Q2_bin]->SetPointError(x_bin, 0, h_sigma_D_red_vs_x[Q2_bin]->GetBinError(x_bin+1));


      float tot_err_sigma_D = sqrt(sigma_D_red_MC_err*sigma_D_red_MC_err + sigma_D_red_MC*sys_err_tot_corr_D*sigma_D_red_MC*sys_err_tot_corr_D);

      sigma_D_vs_x_RC_corr[Q2_bin]->SetPoint(x_bin, x_bin_center*1.1, sigma_D_red_MC);
      sigma_D_vs_x_RC_corr[Q2_bin]->SetPointError(x_bin, 0, tot_err_sigma_D);
      ...
      for( unsigned int pT2_bin = 1; pT2_bin <= h_pT2_bins[x_bin][Q2_bin]->GetNbinsX(); pT2_bin++)
      {
        ...
        h_sigma_n_red[x_bin][Q2_bin]->SetBinError(pT2_bin, h_sigma_n_red[x_bin][Q2_bin]->GetBinError(pT2_bin)*scale_err_MC);
        ...
        h_sigma_n_red_MC_ACC[x_bin][Q2_bin]->SetBinError(pT2_bin, h_sigma_n_red_MC_ACC[x_bin][Q2_bin]->GetBinError(pT2_bin)*scale_err_MC_ACC);
        ...
        h_sigma_n_red_RC[x_bin][Q2_bin]->SetBinError(pT2_bin, h_sigma_n_red_RC[x_bin][Q2_bin]->GetBinError(pT2_bin)*scale_err_RC);
      ...
      sigma_red_D_can->SaveAs(Form("./plots/eN/cross_section/sigma_red_D_Q2bin_%i_x_bin_%i.png", Q2_bin, x_bin));
      sigma_red_D_can->SaveAs(Form("./plots/eN/cross_section/sigma_red_D_Q2bin_%i_x_bin_%i.pdf", Q2_bin, x_bin));

      //_______________________________________________________________________________


      TCanvas *sigma_red_n_can = new TCanvas(Form("sigma_red_n_can_%i_%i",x_bin, Q2_bin), Form("sigma_red_n_can_%i_%i",x_bin, Q2_bin), 1200,1000);
      sigma_red_n_can->cd();
      ...
      default_hist->GetYaxis()->SetTitle("#sigma_{red,n}");
      default_hist->GetYaxis()->CenterTitle();
      default_hist->GetYaxis()->SetRangeUser(0 , 1.1);
      default_hist->SetLineColor(1);
      default_hist->Draw();

      h_sigma_n_red[x_bin][Q2_bin]->Scale( reduce_factor * lumi_factor_MC );
      h_sigma_n_red[x_bin][Q2_bin]->SetMarkerStyle(25);
      h_sigma_n_red[x_bin][Q2_bin]->SetMarkerSize(1.5);
      h_sigma_n_red[x_bin][Q2_bin]->SetMarkerColor(1);
      h_sigma_n_red[x_bin][Q2_bin]->SetLineColor(1);
      h_sigma_n_red[x_bin][Q2_bin]->Draw("p e same");

      //-------------

      h_sigma_n_red_RC[x_bin][Q2_bin]->Scale( reduce_factor * lumi_factor_MC );
      h_sigma_n_red_RC[x_bin][Q2_bin]->SetMarkerStyle(20);
      h_sigma_n_red_RC[x_bin][Q2_bin]->SetMarkerSize(1.5);
      h_sigma_n_red_RC[x_bin][Q2_bin]->SetMarkerColor(kRed);
      h_sigma_n_red_RC[x_bin][Q2_bin]->SetLineColor(kRed);
      if(plot_RC_flag == 1) h_sigma_n_red_RC[x_bin][Q2_bin]->Draw("p e same");


      TF1 *sigma_n_red_RC_fit = new TF1("sigma_n_red_RC_fit", "[0]+[1]*x+[2]*x*x+[3]*x*x*x", 0, 0.01);
      sigma_n_red_RC_fit->SetParameters(0.1, 0.01, -0.1, 0.5);

      TF1 *sigma_n_red_RC_fit_2 = new TF1("sigma_n_red_RC_fit_2", "[0]+[1]*x+[2]*x*x+[3]*x*x*x+[4]*x*x*x*x", 0, 0.01);
      sigma_n_red_RC_fit_2->SetParameters(0.1, 0.01, -0.1, 0.5, 0.5);

      h_sigma_n_red_RC[x_bin][Q2_bin]->Fit(sigma_n_red_RC_fit, "0 R");
      h_sigma_n_red_RC[x_bin][Q2_bin]->Fit(sigma_n_red_RC_fit_2, "0 R");

      sigma_n_red_RC_fit->SetLineColor(1);
      sigma_n_red_RC_fit->SetLineStyle(8);
      if(plot_RC_flag == 1) sigma_n_red_RC_fit->Draw("same");

      sigma_n_red_RC_fit_2->SetLineColor(kRed);
      sigma_n_red_RC_fit_2->SetLineStyle(8);
      //if(plot_RC_flag == 1) sigma_n_red_RC_fit_2->Draw("same");
      ...
      TFitResultPtr fit_ptr = h_sigma_n_red[x_bin][Q2_bin]->Fit(lin_fit, "0 R S");

      double sigma_n_at_pole[1] = { -aT2 };
      double sigma_n_at_pole_err[1];
      fit_ptr->GetConfidenceIntervals(1, 1, 1, sigma_n_at_pole, sigma_n_at_pole_err, 0.683, false); // 0.683 for 1-sigma
      ...
      sigma_free_n->SetPoint(0, -aT2, lin_fit->Eval(sigma_n_at_pole[0]));
      sigma_free_n->SetPointError(0, 0, sigma_n_at_pole_err[0]);
      ...
      sigma_free_n_vs_x[Q2_bin]->SetPoint(x_bin, x_bin_center, lin_fit->Eval(sigma_n_at_pole[0]));
      sigma_free_n_vs_x[Q2_bin]->SetPointError(x_bin, 0, sigma_n_at_pole_err[0]);

      //-------
      TLegend *legend_spectator = new TLegend(0.12, 0.7, 0.32, 0.89);
      legend_spectator->SetTextFont(43);
      legend_spectator->SetTextSize(30);
      legend_spectator->AddEntry(h_sigma_n_red[x_bin][Q2_bin], "True MC", "p e");
      legend_spectator->AddEntry(lin_fit, "Fit to MC", "l");   
      if(plot_RC_flag == 1)
      {
        legend_spectator->AddEntry(h_sigma_n_red_RC[x_bin][Q2_bin], "RC (uncorr.)", "p e");
        legend_spectator->AddEntry(sigma_n_red_RC_fit, "Fit to RC (uncorr.)", "l");
      }
      legend_spectator->AddEntry(sigma_free_n, "Free n (MC)", "p e");
      ...
      sigma_red_n_can->SaveAs(Form("./plots/eN/cross_section/sigma_red_n_Q2bin_%i_x_bin_%i.png", Q2_bin, x_bin));
      sigma_red_n_can->SaveAs(Form("./plots/eN/cross_section/sigma_red_n_Q2bin_%i_x_bin_%i.pdf", Q2_bin, x_bin));

      //_______________________________________________________________________________

      //variation of MC points
      TGraphErrors *sigma_free_n_RC_var[n_variations];

      for(unsigned int var_i = 0; var_i < n_variations; var_i++)
      {
        TH1D *h_sigma_n_red_var = (TH1D*)h_sigma_n_red[x_bin][Q2_bin]->Clone(Form("h_sigma_n_red_xbin_%i_Q2bin_%i_var_%i", x_bin, Q2_bin, var_i));

        for(unsigned int i_point = 0; i_point < h_sigma_n_red_var->GetNbinsX(); i_point++)
        {
          float old_mean = h_sigma_n_red_var->GetBinContent(i_point+1);
          float stat_uncert = h_sigma_n_red_var->GetBinError(i_point+1);

          //float new_mean = my_random->Gaus(old_mean, MC_smear*stat_uncert);
          float new_mean = my_random->Gaus(old_mean, old_mean*sys_err_smear);

          h_sigma_n_red_var->SetBinContent(i_point+1, new_mean);
          h_sigma_n_red_var->SetBinError(i_point+1, stat_uncert);
        }
        ...
        TFitResultPtr fit_ptr_var = h_sigma_n_red_var->Fit(lin_fit_var, "0 R S");


        TH1D *h_eff_nucleon_func_bins_var = (TH1D*)h_sigma_n_red_var->Clone(Form("h_eff_nucleon_func_bins_xbin_%i_Q2bin_%i_var_%i", x_bin, Q2_bin, var_i));
        ...
        for(unsigned int eff_bin = 1; eff_bin <= h_eff_nucleon_func_bins_var->GetNbinsX(); eff_bin++ )
        {
          ...
          h_eff_nucleon_func_bins_var->SetBinContent(eff_bin, sigma_n_red_RC_fit->Eval(bin_center)/lin_fit_var->Eval(bin_center));
          h_eff_nucleon_func_bins_var->SetBinError(eff_bin, 0); //new version

        }


        TH1D *h_sigma_n_red_RC_var = (TH1D*)h_sigma_n_red_RC[x_bin][Q2_bin]->Clone(Form("h_sigma_n_red_RC_xbin_%i_Q2bin_%i_var_%i", x_bin, Q2_bin, var_i));
        h_sigma_n_red_RC_var->Divide(h_eff_nucleon_func_bins_var);
        ...
        TFitResultPtr fit_ptr_var_RC = h_sigma_n_red_RC_var->Fit(lin_fit_var_RC, "0 R S");

        double sigma_n_at_pole_var[1] = { -aT2 };
        double sigma_n_at_pole_err_var[1];
        fit_ptr_var_RC->GetConfidenceIntervals(1, 1, 1, sigma_n_at_pole_var, sigma_n_at_pole_err_var, 0.683, false); // 0.683 for 1-sigma

        sigma_free_n_RC_var[var_i] = new TGraphErrors(1);
        sigma_free_n_RC_var[var_i]->SetNameTitle(Form("sigma_free_n_var_%i", var_i), Form("sigma_free_n_var_%i", var_i));
        sigma_free_n_RC_var[var_i]->SetPoint(0, -aT2, lin_fit_var_RC->Eval(sigma_n_at_pole_var[0]));
        sigma_free_n_RC_var[var_i]->SetPointError(0, 0, sigma_n_at_pole_err_var[0]);
      ...
      eff_nucleon_func_can->cd();
	    ...
	    h_eff_nucleon_func_bins[x_bin][Q2_bin] = (TH1F*)h_sigma_n_red_RC[x_bin][Q2_bin]->Clone(Form("h_eff_nucleon_func_bins_xbin_%i_Q2bin_%i", x_bin, Q2_bin));
      ...
      for(unsigned int eff_bin = 1; eff_bin <= h_eff_nucleon_func_bins[x_bin][Q2_bin]->GetNbinsX(); eff_bin++ )
      {
        ...
        //The shift is calculated from uncorrected - we use the "total" efficiency correction, applied to uncorrected
        h_eff_nucleon_func_bins[x_bin][Q2_bin]->SetBinContent(eff_bin, sigma_n_red_RC_fit->Eval(bin_center)/lin_fit->Eval(bin_center));
      ...
      eff_nucleon_func_can->SaveAs(Form("./plots/eN/eff/eff_nucleon_func_Q2bin_%i_x_bin_%i.png", Q2_bin, x_bin));
      eff_nucleon_func_can->SaveAs(Form("./plots/eN/eff/eff_nucleon_func_Q2bin_%i_x_bin_%i.pdf", Q2_bin, x_bin));


      //--------------------------------------------------------------------------------------------------------------------------

      TCanvas *sigma_red_n_corr_can = new TCanvas(Form("sigma_red_n_corr_can_%i_%i",x_bin, Q2_bin), Form("sigma_red_n_corr_can_%i_%i",x_bin, Q2_bin), 1200,1000);
      sigma_red_n_corr_can->cd();

      //gPad->SetLogy();

      default_hist->Draw();

      h_sigma_n_red_RC[x_bin][Q2_bin]->Divide(h_eff_nucleon_func_bins[x_bin][Q2_bin]);
      h_sigma_n_red_RC[x_bin][Q2_bin]->SetMarkerStyle(20);
      h_sigma_n_red_RC[x_bin][Q2_bin]->SetMarkerSize(1.5);
      h_sigma_n_red_RC[x_bin][Q2_bin]->SetMarkerColor(kRed);
      h_sigma_n_red_RC[x_bin][Q2_bin]->SetLineColor(kRed);
      //h_sigma_n_red_RC[x_bin][Q2_bin]->Draw("p e same");


      TH1D *h_sigma_n_red_RC_stat_sys = (TH1D*)h_sigma_n_red_RC[x_bin][Q2_bin]->Clone(Form("h_sigma_n_red_RC_stat_sys_%i_%i", x_bin, Q2_bin));

      for(unsigned int i_bin = 1; i_bin <= h_sigma_n_red_RC_stat_sys->GetNbinsX(); i_bin++)
      {
        if( h_sigma_n_red_RC_stat_sys->GetBinContent(i_bin) == 0 ) continue;

        float stat_err = h_sigma_n_red_RC_stat_sys->GetBinError(i_bin)/h_sigma_n_red_RC_stat_sys->GetBinContent(i_bin);

        float tot_err = sqrt(stat_err*stat_err + sys_err_tot_corr*sys_err_tot_corr);

        h_sigma_n_red_RC_stat_sys->SetBinError(i_bin, tot_err*h_sigma_n_red_RC_stat_sys->GetBinContent(i_bin));

      }

      h_sigma_n_red_RC_stat_sys->Draw("p e same");
      ...
      TFitResultPtr fit_ptr_RC = h_sigma_n_red_RC[x_bin][Q2_bin]->Fit(lin_fit_RC_corr, "0 R S");

      double sigma_n_at_pole_RC_corr[1] = { -aT2 };
      double sigma_n_at_pole_err_RC_corr[1];
      fit_ptr_RC->GetConfidenceIntervals(1, 1, 1, sigma_n_at_pole_RC_corr, sigma_n_at_pole_err_RC_corr, 0.683, false); // 0.683 for 1-sigma
      ...
      TFitResultPtr fit_ptr_RC_tot_err = h_sigma_n_red_RC_stat_sys->Fit(lin_fit_RC_corr_tot_err, "0 R S");


      double sigma_n_at_pole_RC_corr_tot_err[1] = { -aT2 };
      double sigma_n_at_pole_err_RC_corr_tot_err[1];
      fit_ptr_RC_tot_err->GetConfidenceIntervals(1, 1, 1, sigma_n_at_pole_RC_corr_tot_err, sigma_n_at_pole_err_RC_corr_tot_err, 0.683, false); // 0.683 for 1-sigma

      //-------

      //lin_fit_var_RC_fit->Draw("same"); //for QA of sys. err.

      lin_fit_RC_corr_tot_err->Draw("same");

      TGraphErrors *sigma_free_n_RC_corr = new TGraphErrors(1);
      sigma_free_n_RC_corr->SetNameTitle("sigma_free_n_RC_corr", "sigma_free_n_RC_corr");
      sigma_free_n_RC_corr->SetPoint(0, -aT2, lin_fit_RC_corr_tot_err->Eval(sigma_n_at_pole_RC_corr[0]));
      sigma_free_n_RC_corr->SetPointError(0, 0, sigma_n_at_pole_err_RC_corr_tot_err[0]);
      sigma_free_n_RC_corr->SetMarkerStyle(47);
      sigma_free_n_RC_corr->SetMarkerColor(9);
      sigma_free_n_RC_corr->SetMarkerSize(2);
      sigma_free_n_RC_corr->SetLineColor(9);
      sigma_free_n_RC_corr->Draw("p e same"); //plot last to see point over sys. err.

      //-------------------------------------------------------------------------------

      sigma_free_n_vs_x_RC_corr[Q2_bin]->SetPoint(x_bin, x_bin_center*1.1, lin_fit_RC_corr_tot_err->Eval(sigma_n_at_pole_RC_corr[0]));
      sigma_free_n_vs_x_RC_corr[Q2_bin]->SetPointError(x_bin, 0, sigma_n_at_pole_err_RC_corr_tot_err[0]);

      //-------


      outputFile<<x_bin_center<<" "<<lin_fit_RC_corr_tot_err->Eval(sigma_n_at_pole_RC_corr[0])<<" "<<sigma_n_at_pole_err_RC_corr[0]<<" "<<sigma_n_at_pole_err_RC_corr_tot_err[0]<<" "<<y_mean<<endl;
      ...
      legend_spectator_RC_corr->AddEntry(h_sigma_n_red_RC[x_bin][Q2_bin], "Corrected RC", "p e");
      legend_spectator_RC_corr->AddEntry(lin_fit_RC_corr, "Fit to RC", "l");
      legend_spectator_RC_corr->AddEntry(sigma_free_n_RC_corr, "Free n (RC)", "p e");
      ...
      sigma_red_n_corr_can->SaveAs(Form("./plots/eN/cross_section/sigma_red_n_corr_Q2bin_%i_x_bin_%i.png", Q2_bin, x_bin));
      sigma_red_n_corr_can->SaveAs(Form("./plots/eN/cross_section/sigma_red_n_corr_Q2bin_%i_x_bin_%i.pdf", Q2_bin, x_bin));
    ...
    sigma_red_D_vs_x_can->SaveAs(Form("./plots/eN/cross_section/sigma_red_D_vs_x_Q2bin_%i.png", Q2_bin));
    sigma_red_D_vs_x_can->SaveAs(Form("./plots/eN/cross_section/sigma_red_D_vs_X_Q2bin_%i.pdf", Q2_bin));



    //-----------------------------

    TCanvas *sigma_red_n_vs_x_can = new TCanvas(Form("sigma_red_n_vs_x_can_%i", Q2_bin), Form("sigma_red_n_vs_x_can_%i", Q2_bin), 1200,1000);
    sigma_red_n_vs_x_can->cd();
    ...
    default_hist_vs_x->GetYaxis()->SetTitle("#sigma_{red,n}");
    ...
    if(Q2_bin == 0 ) sigma_free_n_vs_x[Q2_bin]->RemovePoint(9);
    if(Q2_bin == 1 ) sigma_free_n_vs_x[Q2_bin]->RemovePoint(10);
    sigma_free_n_vs_x[Q2_bin]->RemovePoint(Q2_bin);
    if(Q2_bin == 3) sigma_free_n_vs_x[Q2_bin]->RemovePoint(3);
    sigma_free_n_vs_x[Q2_bin]->SetMarkerStyle(25);
    sigma_free_n_vs_x[Q2_bin]->SetMarkerSize(1.5);
    sigma_free_n_vs_x[Q2_bin]->SetMarkerColor(1);
    sigma_free_n_vs_x[Q2_bin]->SetLineColor(1);
    sigma_free_n_vs_x[Q2_bin]->Draw("p e same");

    if(Q2_bin == 0) sigma_free_n_vs_x_RC_corr[Q2_bin]->RemovePoint(9);
    if(Q2_bin == 1 ) sigma_free_n_vs_x_RC_corr[Q2_bin]->RemovePoint(10);
    sigma_free_n_vs_x_RC_corr[Q2_bin]->RemovePoint(Q2_bin);
    if(Q2_bin == 3) sigma_free_n_vs_x_RC_corr[Q2_bin]->RemovePoint(3);
    sigma_free_n_vs_x_RC_corr[Q2_bin]->SetMarkerStyle(20);
    sigma_free_n_vs_x_RC_corr[Q2_bin]->SetMarkerSize(1.5);
    sigma_free_n_vs_x_RC_corr[Q2_bin]->SetMarkerColor(kRed);
    sigma_free_n_vs_x_RC_corr[Q2_bin]->SetLineColor(kRed);
    sigma_free_n_vs_x_RC_corr[Q2_bin]->Draw("p e same");
    ...
    legend_sigma->AddEntry(sigma_free_n_vs_x[Q2_bin], "True MC");
    legend_sigma->AddEntry(sigma_free_n_vs_x_RC_corr[Q2_bin], "Corrected RC");
    legend_sigma->SetBorderSize(0);
    legend_sigma->SetFillColorAlpha(0, 0.01);
    legend_sigma->Draw("same");

    sigma_red_n_vs_x_can->SaveAs(Form("./plots/eN/cross_section/sigma_red_n_vs_x_Q2bin_%i.png", Q2_bin));
    sigma_red_n_vs_x_can->SaveAs(Form("./plots/eN/cross_section/sigma_red_n_vs_x_Q2bin_%i.pdf", Q2_bin));

    //----------------------

    //same plot, but in 4-panel canvas
    sigma_red_n_vs_x_one_can->cd(Q2_bin+1);
    ...
    TCanvas *sigma_red_D_vs_x_graph_can = new TCanvas(Form("sigma_red_D_vs_x_graph_can_%i", Q2_bin), Form("sigma_red_D_vs_x_graph_can_%i", Q2_bin), 1200,1000);
    sigma_red_D_vs_x_graph_can->cd();

    gPad->SetLogx();
    //gPad->SetLogy();

    TH1F *default_D_hist_vs_x = new TH1F(Form("default_D_hist_vs_x_%i", Q2_bin), Form("default_D_hist_vs_x_%i", Q2_bin), 10000, 0, 1);
    default_D_hist_vs_x->GetXaxis()->SetTitle("x_{bj}");
    default_D_hist_vs_x->GetXaxis()->CenterTitle();
    default_D_hist_vs_x->GetXaxis()->SetRangeUser(1e-3 , 0.8);
    default_D_hist_vs_x->GetYaxis()->SetTitle("#sigma_{red,D}");
    default_D_hist_vs_x->GetYaxis()->CenterTitle();
    default_D_hist_vs_x->GetYaxis()->SetRangeUser(0 , 1.0);

    default_D_hist_vs_x->SetLineColor(1);
    default_D_hist_vs_x->Draw();


    if(Q2_bin == 0 ) sigma_D_vs_x[Q2_bin]->RemovePoint(9);
    if(Q2_bin == 1 ) sigma_D_vs_x[Q2_bin]->RemovePoint(10);
    sigma_D_vs_x[Q2_bin]->RemovePoint(Q2_bin);
    if(Q2_bin == 3) sigma_D_vs_x[Q2_bin]->RemovePoint(3);
    sigma_D_vs_x[Q2_bin]->SetMarkerStyle(25);
    sigma_D_vs_x[Q2_bin]->SetMarkerSize(1.5);
    sigma_D_vs_x[Q2_bin]->SetMarkerColor(1);
    sigma_D_vs_x[Q2_bin]->SetLineColor(1);
    sigma_D_vs_x[Q2_bin]->Draw("p e same");

    if(Q2_bin == 0) sigma_D_vs_x_RC_corr[Q2_bin]->RemovePoint(9);
    if(Q2_bin == 1 ) sigma_D_vs_x_RC_corr[Q2_bin]->RemovePoint(10);
    sigma_D_vs_x_RC_corr[Q2_bin]->RemovePoint(Q2_bin);
    if(Q2_bin == 3) sigma_D_vs_x_RC_corr[Q2_bin]->RemovePoint(3);
    sigma_D_vs_x_RC_corr[Q2_bin]->SetMarkerStyle(20);
    sigma_D_vs_x_RC_corr[Q2_bin]->SetMarkerSize(1.5);
    sigma_D_vs_x_RC_corr[Q2_bin]->SetMarkerColor(kRed);
    sigma_D_vs_x_RC_corr[Q2_bin]->SetLineColor(kRed);
    sigma_D_vs_x_RC_corr[Q2_bin]->Draw("p e same");
    ...
    sigma_red_D_vs_x_graph_can->SaveAs(Form("./plots/eN/cross_section/sigma_red_D_vs_x_graph_Q2bin_%i.png", Q2_bin));
    sigma_red_D_vs_x_graph_can->SaveAs(Form("./plots/eN/cross_section/sigma_red_D_vs_x_graph_Q2bin_%i.pdf", Q2_bin));

    //----------------------

    //same plot, but in 4-panel canvasdefault_hist_vs_x
    sigma_red_D_vs_x_one_can->cd(Q2_bin+1);
  ...
  sigma_red_n_vs_x_one_can->SaveAs("./plots/eN/cross_section/sigma_red_n_vs_x.png");
  sigma_red_n_vs_x_one_can->SaveAs("./plots/eN/cross_section/sigma_red_n_vs_x.pdf");

  sigma_red_n_vs_x_one_can_2->SaveAs("./plots/eN/cross_section/sigma_red_n_vs_x_2.png");
  sigma_red_n_vs_x_one_can_2->SaveAs("./plots/eN/cross_section/sigma_red_n_vs_x_2.pdf");


  sigma_red_D_vs_x_one_can->SaveAs("./plots/eN/cross_section/sigma_red_D_vs_x.png");
  sigma_red_D_vs_x_one_can->SaveAs("./plots/eN/cross_section/sigma_red_D_vs_x.pdf");

  sigma_red_D_vs_x_one_can_2->SaveAs("./plots/eN/cross_section/sigma_red_D_vs_x_2.png");
  sigma_red_D_vs_x_one_can_2->SaveAs("./plots/eN/cross_section/sigma_red_D_vs_x_2.pdf");
